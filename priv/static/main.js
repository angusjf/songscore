...
var $author$project$Review$Review = F9(
	function (id, text, stars, user, subject, comments, likes, dislikes, insertedAt) {
		return {comments: comments, dislikes: dislikes, id: id, insertedAt: insertedAt, likes: likes, stars: stars, subject: subject, text: text, user: user};
	});
...
var $author$project$Subject$decoder = A7(
	...
	A2($elm$json$Json$Decode$field, 'spotify_id', $elm$json$Json$Decode$string));
	...
	function (userAndToken, msg) {
		return A2(
			...
			userAndToken.token,
			{
				...
				url: $author$project$Api$apiRoot + ('/feeds/' + userAndToken.user.username)
			});
	});
...
var $author$project$Notification$Notification = F3(
	function (id, text, insertedAt) {
		return {id: id, insertedAt: insertedAt, text: text};
	});
	...
	function (userAndToken, id, msg) {
		return A2(
			...
			userAndToken,
			{
				...
				url: $author$project$Api$apiRoot + ('/reviews/' + $elm$core$String$fromInt(id))
			});
	});
	...
	function (userAndToken, username, msg) {
		return A2(
			...
			userAndToken,
			{
				...
				url: $author$project$Api$apiRoot + ('/users/' + username)
			});
	});
...
var $author$project$Main$init = F3(
	function (_v0, url, key) {
		...
		var initSession = {
			...
			key: key,
			unreadNotifications: $elm$core$Maybe$Nothing,
			userAndToken: uAndT
		};
		var _v1 = $author$project$Widgets$Navbar$init(uAndT);
		var nav = _v1.a;
		var navCmd = _v1.b;
		var start = {navbarModel: nav, page: $author$project$Main$NotFound, session: initSession};
		var _v2 = A2($author$project$Main$stepUrl, url, start);
		var model = _v2.a;
		var cmd = _v2.b;
		var cmds = function () {
			if (uAndT.$ === 'Just') {
...
var $author$project$Widgets$NewReviewForm$convertToReview = F3(
	function (now, form, user) {
		var _v0 = _Utils_Tuple2(form.stars, form.subject);
		if ((_v0.a.$ === 'Just') && (_v0.b.$ === 'Just')) {
			var stars = _v0.a.a;
			var subject = _v0.b.a;
			return $elm$core$Maybe$Just(
				{comments: _List_Nil, dislikes: _List_Nil, id: $elm$core$Maybe$Nothing, insertedAt: now, likes: _List_Nil, stars: stars, subject: subject, text: form.text, user: user});
		} else {
			return $elm$core$Maybe$Nothing;
		}
	});
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'image',
				$elm$json$Json$Encode$string(subject.image)),
				...
				_Utils_Tuple2(
				'artist',
				$elm$json$Json$Encode$string(subject.artist)),
				_Utils_Tuple2(
				'spotify_id',
				$elm$json$Json$Encode$string(subject.spotifyId))
			]));
};
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'inserted_at',
				$rtfeldman$elm_iso8601_date_strings$Iso8601$encode(review.insertedAt))
			]));
};
	...
	function (userAndToken, review, msg) {
		return A2(
			...
			userAndToken.token,
			{
				...
				expect: A2($elm$http$Http$expectJson, msg, $author$project$Review$decoder),
				url: $author$project$Api$apiRoot + '/reviews'
			});
	});
var $author$project$Widgets$NewReviewForm$GotResults = function (a) {
	return {$: 'GotResults', a: a};
};
var $author$project$Widgets$NewReviewForm$minSearchLength = 2;
var $author$project$Widgets$NewReviewForm$resultToSubject = function (result) {
	return {
		artist: result.artist,
		id: $elm$core$Maybe$Nothing,
		image: result.imageUrl,
		kind: (result.kind === 'Album') ? $author$project$Subject$Album : $author$project$Subject$Song,
		spotifyId: result.spotifyId,
		title: result.name
	};
};
var $author$project$Widgets$NewReviewForm$resultsLimit = 4;
...
var $author$project$Api$SubjectResult = F6(
	function (name, artist, imageUrl, imageUrlSmall, kind, spotifyId) {
		return {artist: artist, imageUrl: imageUrl, imageUrlSmall: imageUrlSmall, kind: kind, name: name, spotifyId: spotifyId};
	});
var $elm$json$Json$Decode$at = F2(
	function (fields, decoder) {
		return A3($elm$core$List$foldr, $elm$json$Json$Decode$field, decoder, fields);
	});
var $author$project$Api$subjectResultsDecoder = A2(
	...
	$elm$json$Json$Decode$list(
		A7(
			$elm$json$Json$Decode$map6,
			$author$project$Api$SubjectResult,
...
var $author$project$Api$searchSubjects = F3(
	function (query, limit, msg) {
		...
		return $elm$http$Http$get(
			{
				expect: A2($elm$http$Http$expectJson, msg, $author$project$Api$subjectResultsDecoder),
				url: _Utils_ap($author$project$Api$apiRoot, url)
			});
	});
		...
		switch (msg.$) {
			...
			case 'OnSubjectQueryChanged':
				...
				return (_Utils_cmp(
					...
					_Utils_update(
						model,
						{results: _List_Nil, subjectQuery: _new}),
					...
					A3(
						...
						function (x) {
							return model.toOuterMsg(
								$author$project$Widgets$NewReviewForm$GotResults(x));
						}));
			case 'GotResults':
				var res = msg.a;
				if (res.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								results: A2($elm$core$List$take, $author$project$Widgets$NewReviewForm$resultsLimit, results)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var e = res.a;
					return _Debug_todo(
						'Widgets.NewReviewForm',
						{
					...
					_Utils_update(
						model,
						{
							...
							subject: $elm$core$Maybe$Just(
								$author$project$Widgets$NewReviewForm$resultToSubject(track)),
							subjectQuery: ''
						}),
					$elm$core$Platform$Cmd$none);
			default:
...
var $author$project$Api$deleteReview = F3(
	function (uAndT, review, msg) {
		...
		return A2(
			...
			{
				expect: A2($elm$http$Http$expectJson, msg, $author$project$Review$decoder),
				url: $author$project$Api$apiRoot + ('/reviews/' + id)
			});
	});
	...
	function (uAndT, review, comment, msg) {
		...
		return A2(
			...
			uAndT.token,
			{
				...
				expect: A2($elm$http$Http$expectJson, msg, $author$project$Review$decoder),
				url: $author$project$Api$apiRoot + ('/reviews/' + (id + '/comments'))
			});
	});
	...
	function (uAndT, review, msg) {
		...
		return A2(
			...
			uAndT.token,
			{
				body: $elm$http$Http$emptyBody,
				expect: A2($elm$http$Http$expectJson, msg, $author$project$Review$decoder),
				url: $author$project$Api$apiRoot + ('/reviews/' + (id + '/dislike'))
			});
	});
	...
	function (uAndT, review, msg) {
		...
		return A2(
			...
			uAndT.token,
			{
				body: $elm$http$Http$emptyBody,
				expect: A2($elm$http$Http$expectJson, msg, $author$project$Review$decoder),
				url: $author$project$Api$apiRoot + ('/reviews/' + (id + '/like'))
			});
	});
			...
			case 'CommentSubmitted':
				...
				if (result.$ === 'Ok') {
					...
					return _Debug_todo(
						'Widgets.ReviewList',
						{
							start: {line: 98, column: 18},
							end: {line: 98, column: 28}
						})(
						$elm$core$Debug$toString(e));
				}
			default:
	...
	function (msg, model, session) {
		switch (msg.$) {
			case 'OnNRFSubmitPressed':
				...
				if (_v1.$ === 'Just') {
					var uAndT = _v1.a;
					var _v2 = A3($author$project$Widgets$NewReviewForm$convertToReview, session.currentTime, model.nrf, uAndT.user);
					if (_v2.$ === 'Just') {
						var newReview = _v2.a;
						return _Utils_Tuple3(
							model,
		...
		return $elm$http$Http$post(
			{
				...
				url: $author$project$Api$apiRoot + '/auth'
			});
	});
...
var $author$project$Session$storePort = _Platform_outgoingPort(
	'storePort',
	function ($) {
		var a = $.a;
		var b = $.b;
...
var $author$project$Session$store = function (data) {
	return $author$project$Session$storePort(
		_Utils_Tuple2(
			data.userAndToken,
			$elm$time$Time$posixToMillis(data.currentTime)));
};
			...
			if (result.$ === 'Ok') {
				...
				return _Utils_Tuple3(
					...
					_Utils_update(
						session,
						{
							unreadNotifications: $elm$core$Maybe$Just(
								!$elm$core$List$isEmpty(_new))
						}),
					$elm$core$Platform$Cmd$none);
			} else {
				var e = result.a;
...
var $author$project$Api$getUsernameAvailability = F2(
	function (username, msg) {
		return $elm$http$Http$get(
			{
				expect: A2($elm$http$Http$expectJson, msg, $elm$json$Json$Decode$bool),
				url: $author$project$Api$apiRoot + ('/users/' + (username + '/available'))
			});
	});
			...
			case 'UsernameChanged':
				...
				return _Utils_Tuple3(
					_Utils_update(
						...
						{username: _new}),
					session,
					($elm$core$String$length(_new) > 1) ? A2($author$project$Api$getUsernameAvailability, _new, $author$project$Pages$Register$GotUsernameAvailable) : $elm$core$Platform$Cmd$none);
			case 'PasswordChanged':
				var _new = msg.a;
				return _Utils_Tuple3(
					_Utils_update(
...
var $author$project$Pages$Settings$update = F3(
	function (msg, model, session) {
		switch (msg.$) {
			case 'UsernameChanged':
				var _new = msg.a;
				return ((!_Utils_eq(_new, model.oldUsername)) && ($elm$core$String$length(_new) > 1)) ? _Utils_Tuple3(
					_Utils_update(
						model,
...
var $author$project$Styles$background = A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1);
...
var $author$project$Widgets$NewReviewForm$OnSubjectClear = {$: 'OnSubjectClear'};
...
var $author$project$Styles$roundedTL = $mdgriffith$elm_ui$Element$Border$roundEach(
	{bottomLeft: 0, bottomRight: 0, topLeft: 8, topRight: 0});
...
var $author$project$Widgets$NewReviewForm$OnResultClicked = function (a) {
	return {$: 'OnResultClicked', a: a};
};
var $author$project$Styles$squareTiny = _List_fromArray(
	[
		$mdgriffith$elm_ui$Element$width(
...
var $author$project$Widgets$NewReviewForm$viewResult = F2(
	function (toOuterMsg, result) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$button,
			_List_fromArray(
				[
				...
				label: A2(
					...
					_List_fromArray(
						[
							A2(
							$mdgriffith$elm_ui$Element$image,
							$author$project$Styles$squareTiny,
							{description: '', src: result.imageUrlSmall}),
							A2(
							$mdgriffith$elm_ui$Element$column,
							_List_fromArray(
								[
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$author$project$Styles$text(result.name)
										])),
									A2(
									$mdgriffith$elm_ui$Element$paragraph,
									_List_Nil,
									_List_fromArray(
										[
											$author$project$Styles$text(result.artist)
										]))
								]))
						])),
				onPress: $elm$core$Maybe$Just(
					toOuterMsg(
						$author$project$Widgets$NewReviewForm$OnResultClicked(result)))
			});
	});
var $author$project$Widgets$NewReviewForm$viewSubjectForm = function (form) {
	...
	var results = A2(
		...
		A2(
			$elm$core$List$map,
			$author$project$Widgets$NewReviewForm$viewResult(form.toOuterMsg),
			form.results));
	var _v0 = form.subject;
	if (_v0.$ === 'Just') {
		var subject = _v0.a;
		return A2(
			$mdgriffith$elm_ui$Element$column,
			...
			_List_fromArray(
				[
					...
					'clear',
					$elm$core$Maybe$Just(
						form.toOuterMsg($author$project$Widgets$NewReviewForm$OnSubjectClear)))
				]));
	} else {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			...
			$elm$core$List$isEmpty(form.results) ? _List_fromArray(
				[searchBox]) : _List_fromArray(
				[searchBox, results]));
	}
};
...
var $author$project$Styles$Guest = {$: 'Guest'};
var $author$project$Styles$Mine = {$: 'Mine'};
...
var $author$project$Styles$joinAllNice = function (joinme) {
	var helper = function (strs) {
		if (strs.b) {
			if (strs.b.b) {
				if (!strs.b.b.b) {
					var x = strs.a;
	...
	return helper(
		$elm_community$list_extra$List$Extra$unique(joinme));
};
...
var $author$project$Styles$longAgo = F2(
	function (now, insertedAt) {
		return A2($ryannhg$date_format$DateFormat$Relative$relativeTime, now, insertedAt);
	});
	...
	return A2(
		...
		_List_Nil,
		{
			label: A2(
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Background$color($author$project$Styles$spotifyGreen),
						$mdgriffith$elm_ui$Element$Font$color($author$project$Styles$white),
						$mdgriffith$elm_ui$Element$Font$bold,
						$author$project$Styles$paddingSmall,
						$author$project$Styles$roundedSmall,
		...
		var nameAndText = A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Styles$text(
					A2($author$project$Styles$longAgo, now, review.insertedAt))
				]));
		var likes = A2(
			$elm$core$List$map,
			function (u) {
	...
	function (session, model) {
		...
		var notifs = A2(
			...
			_Utils_ap(
				...
				A2($elm$core$Maybe$withDefault, false, session.unreadNotifications) ? _List_fromArray(
					[
